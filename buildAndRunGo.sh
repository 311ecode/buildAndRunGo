  ...
  if command -v go &>/dev/null; then
    ...
    if [[ "${!build_always_var:-0}" == "0" ]]; then
      ...
      if [[ -f "$exec_binary" ]]; then
        [[ "${DEBUG}" == "true" ]] && echo "Binary $exec_binary already exists and $build_always_var is 0, skipping build." >&2
        should_build=false
      else
        [[ "${DEBUG}" == "true" ]] && echo "Binary $exec_binary not found, building..." >&2
      fi
    else
      [[ "${DEBUG}" == "true" ]] && echo "$build_always_var is set to ${!build_always_var}, forcing build..." >&2
    fi

    # Build only if needed
    if [[ "$should_build" == "true" ]]; then
      [[ "${DEBUG}" == "true" ]] && echo "Go is installed. Starting build..." >&2
      ...
      [[ "${DEBUG}" == "true" ]] && echo "Compilation process finished in $duration seconds." >&2
    fi
  else
    [[ "${DEBUG}" == "true" ]] && echo "Go is not installed. Attempting to use existing binary for $exec_os/$exec_arch..." >&2
  fi

  # Check if the binary exists
  if [[ ! -f $exec_binary ]]; then
    if [[ -f "$bin_dir/$program_name" ]]; then
      [[ "${DEBUG}" == "true" ]] && echo "Found generic binary $bin_dir/$program_name, using it instead" >&2
